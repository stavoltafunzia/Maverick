...
void RK1Ocp2NlpSinglePhase::setupForNlpConstraintsJacobianMatrixes() {
  ...
  copyVectorTo(fo_eqns_j_xu_mat.outerIndexPtr(), _p_fo_eqns_j_xu_outer_start,
               (integer) fo_eqns_j_xu_mat.outerSize() + 1);
  ...
  copyVectorTo(fo_eqns_j_dxu_mat.outerIndexPtr(), _p_fo_eqns_j_dxu_outer_start,
               (integer) fo_eqns_j_dxu_mat.outerSize() + 1);
  ...
  copyVectorTo(path_constr_j_xu_mat.outerIndexPtr(), _p_path_constr_j_xu_outer_start,
               (integer) path_constr_j_xu_mat.outerSize() + 1);
  ...
  copyVectorTo(path_constr_j_dxu_mat.outerIndexPtr(), _p_path_constr_j_dxu_outer_start,
               (integer) path_constr_j_dxu_mat.outerSize() + 1);
  ...
  copyVectorTo(int_constr_j_xu_mat.outerIndexPtr(), _p_int_constr_j_xu_outer_start,
               (integer) int_constr_j_xu_mat.outerSize() + 1);
  ...
  copyVectorTo(int_constr_j_dxu_mat.outerIndexPtr(), _p_int_constr_j_dxu_outer_start,
               (integer) int_constr_j_dxu_mat.outerSize() + 1);
  ...
  // boundary conditions
  nnz = _ocp_problem.boundaryConditionsJacXuInitNnz(_i_phase) + _ocp_problem.boundaryConditionsJacXuFinNnz(_i_phase) +
        _ocp_problem.boundaryConditionsJacPNnz(_i_phase);
...
void RK1Ocp2NlpSinglePhase::calculateNlpConstraintsJacobainPattern(integer const fo_eqns_j_y_rows[],
  ...
  for (integer mesh_interval = 0; mesh_interval < _p_mesh->getNumberOfDiscretisationPoints(); mesh_interval++) {
    if (mesh_interval <
        _p_mesh->getNumberOfIntervals()) { //the first order equations and path constraints do not apply to the last mesh points
...
void RK1Ocp2NlpSinglePhase::calculateNlpConstraintsJacobianMatrixY(ocpStateAtInterval const & ocp_state, real values[]) const {
  ...
  multiplyAndCopyVectorTo((real *) fo_eqns_j_y_left_mat.valuePtr(), p_current_value_ptr, _p_scale_factor_fo_eqns_j_y,
                          _fo_eqns_j_y_nnz); //TODO: remove cast
  ...
  multiplyAndCopyVectorTo((real *) fo_eqns_j_y_right_mat.valuePtr(), p_current_value_ptr, _p_scale_factor_fo_eqns_j_y,
                          _fo_eqns_j_y_nnz); //TODO: remove cast
  p_current_value_ptr += _fo_eqns_j_y_nnz;
  ...
  multiplyAndCopyVectorTo((real *) path_constr_j_y_left_mat.valuePtr(), p_current_value_ptr,
                          _p_scale_factor_path_constr_j_y, _path_constr_j_y_nnz);
  ...
  // seventh block
  multiplyAndCopyVectorTo((real *) path_constr_j_y_right_mat.valuePtr(), p_current_value_ptr,
                          _p_scale_factor_path_constr_j_y, _path_constr_j_y_nnz); //TODO: remove cast
  p_current_value_ptr += _path_constr_j_y_nnz;
...
void RK1Ocp2NlpSinglePhase::calculateNlpConstraintsJacobianMatrixF(real const initial_state_control[],
  ...
  _ocp_problem.boundaryConditionsJac(_i_phase, initial_state_control, final_state_control, parameters, initial_zeta,
                                     final_zeta,
                                     p_jac_xu_ini,
                                     p_jac_xu_fin,
                                     p_jac_p);
