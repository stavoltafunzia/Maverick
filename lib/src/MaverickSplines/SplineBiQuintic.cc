...
namespace Splines {
	...
	BiQuinticSpline::makeSpline() {
		...
		for (sizeType j = 0; j < ny; ++j) {
			...
			for (sizeType i = 0; i < nx; ++i) {
				DXY[ipos_C(i, j)] += sp.ypNode(i);
				DXY[ipos_C(i, j)] /= 2;
				DXXY[ipos_C(i, j)] += sp.yppNode(i);
				DXXY[ipos_C(i, j)] /= 2;
				DXYY[ipos_C(i, j)] += sp1.ypNode(i);
				DXYY[ipos_C(i, j)] /= 2;
				DXXYY[ipos_C(i, j)] += sp1.yppNode(i);
				DXXYY[ipos_C(i, j)] /= 2;
			}
		}
	...
	char const *
	BiQuinticSpline::type_name() const { return "BiQuintic"; }
