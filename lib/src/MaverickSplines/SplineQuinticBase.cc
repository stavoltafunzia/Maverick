...
//    npts = lastInterval = 0 ;
	...
	QuinticSplineBase::coeffs(valueType cfs[], valueType nodes[], bool transpose) const {
		...
		for (sizeType i = 0; i < n; ++i) {
			...
			valueType c = Ypp[i] / 2;
			valueType d =
							((10 * (Y[i + 1] - Y[i]) / H - 6 * Yp[i] - 4 * Yp[i + 1]) / H - 1.5 * Ypp[i] + 0.5 * Ypp[i + 1]) / H;
			valueType e =
							((15 * (Y[i] - Y[i + 1]) / H + 8 * Yp[i] + 7 * Yp[i + 1]) / H + 1.5 * Ypp[i] - Ypp[i + 1]) / (H * H);
			valueType f = ((6 * (Y[i + 1] - Y[i]) / H - 3 * (Yp[i] + Yp[i + 1])) / H - 0.5 * Ypp[i] + 0.5 * Ypp[i + 1]) /
										(H * H * H);
			if (transpose) {
				cfs[6 * i + 5] = a;
				cfs[6 * i + 4] = b;
