...
namespace Splines {
	...
	SplineSet::setup(GenericContainer const &gc) {
		...
		*/
		...
		if (GC_MAT_REAL == gc_ydata.get_type()) {
			...
			SPLINE_ASSERT(false,
										"[SplineSet[" << _name
																	<< "]::setup] field `data` expected to be of type `mat_real_type`, `vector_type` or `map_type' found: ` "
																	<<
	...
	void
	SplineSet::eval2(vec_real_type const &zetas, sizeType indep, vec_string_type const &columns,
	...
	*/
	...
	//! Evaluate all the splines at `zeta` values contained in vec and fill a map of vector in a GenericContainer with keys in `columns` and `indep` as independent spline
	void
	SplineSet::eval2_D(vec_real_type const &zetas, sizeType indep, vec_string_type const &columns,
										 GenericContainer &gc) const {
		sizeType npts = sizeType(zetas.size());
		map_type &vals = gc.set_map();
	...
	*/
	...
	//! Evaluate all the splines at `zeta` values contained in vec and fill a map of vector in a GenericContainer with keys in `columns` and `indep` as independent spline
	void
	SplineSet::eval2_DD(vec_real_type const &zetas, sizeType indep, vec_string_type const &columns,
											GenericContainer &gc) const {
		sizeType npts = sizeType(zetas.size());
		map_type &vals = gc.set_map();
	...
	*/
	...
	//! Evaluate all the splines at `zeta` values contained in vec and fill a map of vector in a GenericContainer with keys in `columns` and `indep` as independent spline
	void
	SplineSet::eval2_DDD(vec_real_type const &zetas, sizeType indep, vec_string_type const &columns,
											 GenericContainer &gc) const {
		sizeType npts = sizeType(zetas.size());
		map_type &vals = gc.set_map();
