...
//    }
	...
	sizeType Spline::search(valueType x) const {
...
#endif
		sizeType interval_index = sizeType(lower_bound(X, X + npts, x) - X);
		if (interval_index > 0) --interval_index;
		if (X[interval_index] == X[interval_index + 1]) ++interval_index; // degenerate interval for duplicated nodes
		if (interval_index + 1 >= npts) interval_index = npts - 2;
