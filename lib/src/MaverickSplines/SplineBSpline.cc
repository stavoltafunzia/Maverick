...
namespace Splines {
	...
	BSpline<_degree>::sample_bases(indexType nx,
		...
		II.clear();
		II.reserve(sizeType(nx));
		JJ.clear();
		JJ.reserve(sizeType(nx));
		vals.clear();
		vals.reserve(sizeType(nx));
...
#endif
		// 0 1 2 3
		sizeType interval_index = sizeType(lower_bound(knots + _degree, knots + npts + 1, x) - knots);
		if (interval_index > _degree) --interval_index;

		return interval_index - _degree;
	}
