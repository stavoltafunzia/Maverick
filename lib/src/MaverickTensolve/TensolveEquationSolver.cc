...
TensolveEquationSolver::TensolveEquationSolver(EquationSolverSupplierInterface const &problem) :
    ...
    EquationSolver(_problem.getNumEquations(), _problem.getNumUnknowns(),
                   [this](Maverick::real const x[], Maverick::real f[], integer m, integer n) {
                     this->evalFunctions(x, f, m, n);
                   }, [this](Maverick::real const x[], Maverick::real jac[], integer m, integer n,
                             integer max_m) { this->evalJacobian(x, jac, m, n, max_m); }) {
...
void TensolveEquationSolver::setMaxIterations(integer const max_iterations) {
  ...
  if (max_iterations < 0) {
    ...
    maverick.Log(InfoLevel::info_level_warning,
                 "TensolveEquationSolver::setMaxIterations: negative number of maximum iterations not allowed. Will use the deualt one: " +
                 std::to_string(DEFUALT_TENSOLVE_ITERATIONS) + "\n");
...
void TensolveEquationSolver::evalFunctions(real const x[], real f[], integer const num_equations,
                                           integer const num_unknowns) const {
...
void TensolveEquationSolver::evalJacobian(real const x[], real tensolve_jac[], integer const num_equations,
                                          integer const num_unknowns, integer const max_m) const {
  real new_x[num_unknowns];
  copyVectorTo(x, new_x, num_unknowns);
