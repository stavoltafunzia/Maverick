cdef extern from "MaverickCore/MaverickCInterface.h":
    ...
    # MAVERICK SOLVER
    cdef void   loadMaverickOcpSolver(int maverick_ocp_id, int * const p_err_code);
    cdef void   unloadMaverickOcpSolver(int maverick_ocp_id, int * const p_err_code);
...
class Solver:
    ...
    def close(self):
        cdef int err_code = 0
        unloadMaverickOcpSolver(self.__ocp_id, & err_code);
        if (err_code != 0):
    ...
    def __loadByLibName(self, ocp_name):
        ...
        loadMaverickOcpSolver(self.__ocp_id, &err_code);
        if err_code is 1:
