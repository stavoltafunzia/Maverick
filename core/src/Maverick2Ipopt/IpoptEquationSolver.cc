...
IpoptEquationSolver::IpoptEquationSolver( EquationSolverSupplierInterface const & problem ) : EquationSolverInterface(problem) {
...
void IpoptEquationSolver::setMaxIterations(integer const max_iterations) {
    if (max_iterations < 0) {
        ...
        maverick.Log(InfoLevel::info_level_warning, "IpoptEquationSolver::setMaxIterations: negative number of maximum iterations not allowed. Will use the deualt one: " + std::to_string(DEFUALT_IPOPT_ITERATIONS) + "\n");
    } else {
