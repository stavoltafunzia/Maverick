...
//        npts = lastInterval = 0 ;
    ...
    CubicSplineBase::operator () ( valueType x ) const {
        ...
        valueType _base[4];
        Hermite3( x-X[i], X[i+1]-X[i], _base ) ;
        return _base[0] * Y[i]   +
        _base[1] * Y[i+1] +
        _base[2] * Yp[i]  +
        _base[3] * Yp[i+1] ;
    ...
    CubicSplineBase::D( valueType x ) const {
        sizeType i = Spline::search( x ) ;
        valueType _base_D[4];
        Hermite3_D( x-X[i], X[i+1]-X[i], _base_D ) ;
        return _base_D[0] * Y[i]   +
        _base_D[1] * Y[i+1] +
        _base_D[2] * Yp[i]  +
        _base_D[3] * Yp[i+1] ;
    }
    ...
    CubicSplineBase::DD( valueType x ) const {
        sizeType i = Spline::search( x ) ;
        valueType _base_DD[4];
        Hermite3_DD( x-X[i], X[i+1]-X[i], _base_DD ) ;
        return _base_DD[0] * Y[i]   +
        _base_DD[1] * Y[i+1] +
        _base_DD[2] * Yp[i]  +
        _base_DD[3] * Yp[i+1] ;
    }
    ...
    CubicSplineBase::DDD( valueType x ) const {
        sizeType i = Spline::search( x ) ;
        valueType _base_DDD[4];
        Hermite3_DDD( x-X[i], X[i+1]-X[i], _base_DDD ) ;
        return _base_DDD[0] * Y[i]   +
        _base_DDD[1] * Y[i+1] +
        _base_DDD[2] * Yp[i]  +
        _base_DDD[3] * Yp[i+1] ;
    }
