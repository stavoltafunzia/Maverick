...
static std::map<int, OcpSolver *>              map_solver;
...
void   deleteMaverickOcp(void * const p_maverick_ocp, int * const p_err_code) {
    ...
    * p_err_code = 0;
    ...
    if (it != map_open_lib.end()) { // if the pointer is found
        ...
        for (std::map<MaverickOcp *, void *>::iterator xx = map_open_lib.begin(); xx != map_open_lib.end(); xx++) { // check if we should close the library
...
/* create a MaverickSolver object binded to the MaverickOcp object with the provided ID. Only one solver
 ...
 -1 = error in creating the object (e.g. not enough available) memory
 */
int   loadMaverickSolver(int maverick_ocp_id, int * const p_err_code) {
    ...
    for (std::map<int, OcpSolver *>::iterator its = map_solver.begin(); its != map_solver.end(); its++) {
    ...
    // now proceed
    * p_err_code = -1;
    OcpSolver * solver = (OcpSolver *) getMaverickSolver(it->second, p_err_code);
    if (*p_err_code != 0)
        return 0;

    *p_err_code = -1;
    map_solver.insert( std::pair<int, OcpSolver *>(id, solver) );
    *p_err_code = 0;

    return id;
}

/* delete the MaverickSolver object with the provided ID.
 Error codes are: 0 = succsefull
 1 = ID not found (object non existing)
 */
void   unloadMaverickSolver(int maverick_ocp_id, int * const p_err_code) {
    std::map<int, OcpSolver *>::iterator it = map_solver.find(maverick_ocp_id);
...
/* create a MaverickSolver object binded to the MaverickOcp object provided.
 The object pointer is returned the management is left to the user.
 Error codes are: 0 = succsefull
 1 = null MaverickOcp pointer
 -1 = error in creating the object (e.g. not enough available) memory
 */
void * getMaverickSolver(void * const p_maverick_ocp, int * const p_err_code) {
    ...
    *p_err_code = -1;
    MaverickOcp * ocp = (MaverickOcp *) p_maverick_ocp;
    OcpSolver * solver = new OcpSolver(* ocp);
    *p_err_code = 0;
    return (void *) solver;
}
...
/* delete the MaverickSolver object with the provided ID.
 ...
 1 = null pointer
 */
void   deleteMaverickSolver(void * solver, int * const p_err_code) {
    OcpSolver * _solver = (OcpSolver *) solver;
    if (_solver == NULL) {
        * p_err_code = 1;
        return;
    }
 ...
 */
void   solveFromInternalGC(int maverick_id, int input_generic_container_id, int sol_type, void * p_output_generic_container,
...
/* Search the maverick solver with the ID specified and calls the 'solve' method passing the GenericContainer
 ...
 5 = error in writing the solution to the output generic container
 */
void   solveFromExternalGC(int maverick_id, void * p_input_generic_container, int sol_type, void * p_output_generic_container,
                         int * const p_err_code,
                         char error[], size_t error_length) {

    // check if solver exists
    std::map<int, OcpSolver *>::iterator it = map_solver.find(maverick_id);
    if (it == map_solver.end()) {
        * p_err_code = 1;
        return;
    }
...
/* call the 'solve' method to the solver passed through pointer and pass as argument the GenericContainer
 ...
 5 = error in writing the solution to the output generic container
 */
void   solveFromExternalSolver(void * p_maverick_solver, void * p_input_generic_container, int sol_type, void * p_output_generic_container,
                             int * const p_err_code,
                             char error[], size_t error_length) {
    ...
    // solve
    try {
        out = ((MaverickSolver *) p_maverick_solver)->solve( *((GC::GenericContainer *) p_input_generic_container));
    ...
    if (sol_type == 0)
        ...
        out.writeContentToGC(*((GC::GenericContainer *) p_output_generic_container),
                             &(((MaverickSolver *) p_maverick_solver)->getOcpProblem()) );
}
