...
void OcpScaling::setupForOcp( GC::GenericContainer const & gc, MaverickOcp const & ocp_problem, vec_2d_real const & zeta ) {
    ...
    chechForAllPhasesDeclarationInGc( gc, ocp_problem.numberOfPhases(), are_all_phases_declared, is_one_phase_declared, missing_phases );

    if ( (!is_one_phase_declared) && (ocp_problem.numberOfPhases()==1) ) { // only in this case try to check if it is declared as root value
...
void OcpScaling::setupForOcpAsNoScaling( MaverickOcp const & ocp_problem ) {
    ...
    for (integer i_phase = 0; i_phase < ocp_problem.numberOfPhases(); i_phase++) {
        _states_control.push_back( vec_1d_real( ocp_problem.numberOfStates(i_phase)+ocp_problem.numberOfControls(i_phase), to_push ) );
        _algebraic_states_controls.push_back( vec_1d_real( ocp_problem.numberOfAlgebraicStates(i_phase)+ocp_problem.numberOfAlgebraicControls(i_phase), to_push ) );
...
void OcpScaling::writeScalingsToStream( std::ostream & out ) const {
    ...
    for (integer i_phase=0; i_phase<_states_control.size(); i_phase++) {
        ...
        if (_states_control[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _states_control[i_phase].begin(); it!=_states_control[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_algebraic_states_controls[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _algebraic_states_controls[i_phase].begin(); it!=_algebraic_states_controls[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_point_constraints[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _point_constraints[i_phase].begin(); it!=_point_constraints[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_path_constraints[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _path_constraints[i_phase].begin(); it!=_path_constraints[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_int_constraints[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _int_constraints[i_phase].begin(); it!=_int_constraints[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_boundary_conditions[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _boundary_conditions[i_phase].begin(); it!=_boundary_conditions[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
        ...
        if (_event_constraints[i_phase].size()>0) {
            ...
            for (vec_1d_real::const_iterator it = _event_constraints[i_phase].begin(); it!=_event_constraints[i_phase].end(); it++) {
                out << *it << ", ";
            }
            out << "\n";
        }
    ...
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_lagrange_by_n"       << (_options.multiply_lagrange_by_n      ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_int_constr_by_n"     << (_options.multiply_int_constr_by_n    ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_foeqns_by_dz"        << (_options.multiply_foeqns_by_dz       ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_foeqns_by_n"         << (_options.multiply_foeqns_by_n        ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "divide_foeqns_by_z"           << (_options.divide_foeqns_by_z          ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_path_constr_by_dz"   << (_options.multiply_path_constr_by_dz  ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_path_constr_by_n"    << (_options.multiply_path_constr_by_n   ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "divide_path_constr_by_z"      << (_options.divide_path_constr_by_z     ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_point_constr_by_dz"  << (_options.multiply_point_constr_by_dz ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "multiply_point_constr_by_n"   << (_options.multiply_point_constr_by_n  ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "divide_point_constr_by_z"     << (_options.divide_point_constr_by_z    ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "divide_mayer_by_n"            << (_options.divide_mayer_by_n           ? "true" : "false")  << "\n";
    out << "\t" << std::left << std::setfill(spacer) << std::setw(width) << "divide_bcs_by_n"              << (_options.divide_bcs_by_n             ? "true" : "false")  << "\n";
...
void OcpScaling::setupForOcpAndOnePhase( GC::GenericContainer const & gc, MaverickOcp const & ocp_problem, vec_2d_real const & all_zeta ) {
    ...
    string option_not_rec = "Ocp scaling option xxx for phase " + std::to_string(i_phase) + " is not recognised. Possible options are: 'automatic-bounds'\n";

    {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong states and controls scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<ocp_problem.numberOfStates(i_phase); i++) {
                MAVERICK_ASSERT(states_controls[i]>0,"State scaling for state at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
            for (integer i=0; i<ocp_problem.numberOfControls(i_phase); i++) {
                MAVERICK_ASSERT(states_controls[i+ocp_problem.numberOfStates(i_phase)]>0,"Scaling for control at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
            ...
            if (found) {
                ...
                _states_control.push_back( vec_1d_real(ocp_problem.numberOfStates(i_phase) + ocp_problem.numberOfControls(i_phase) , 1) );
            }
        }
    }

    {
        ...
        if (found) {
            ...
            integer exp = (integer) ocp_problem.numberOfAlgebraicStates(i_phase) + (integer)  ocp_problem.numberOfAlgebraicControls(i_phase);
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong algebraic states and controls scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<ocp_problem.numberOfAlgebraicStates(i_phase); i++) {
                MAVERICK_ASSERT(algebraic_states_controls[i]>0,"Algebraic state scaling for state at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
            for (integer i=0; i<ocp_problem.numberOfAlgebraicControls(i_phase); i++) {
                MAVERICK_ASSERT(algebraic_states_controls[i+ocp_problem.numberOfAlgebraicStates(i_phase)]>0,"Scaling for algebraic control at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
            ...
            if (found) {
                found = checkIfStringMatchesAutomaticBounds(str);
                if (found) {
                    integer const nnz = ocp_problem.numberOfAlgebraicStates(i_phase) + ocp_problem.numberOfAlgebraicControls(i_phase);
                    real lower_bounds[nnz];
                    real upper_bounds[nnz];
                ...
                _algebraic_states_controls.push_back( vec_1d_real(ocp_problem.numberOfAlgebraicStates(i_phase) + ocp_problem.numberOfAlgebraicControls(i_phase) , 1) );
            }
        }
    }
    ...
    if (ocp_problem.numberOfParameters(i_phase)>0) {
        ...
        if (found) {
            ...
            for (integer i=0; i<params.size(); i++) {
                MAVERICK_ASSERT(params[i]>0,"Scaling for parameter at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
...

    {
        ...
        if (found) {
            ...
            if (found) {
                found = checkIfStringMatchesAutomaticBounds(str);
                if (found) {
                    str = "Ocp scaling of type 'automatic-bounds' for mayer target for phase " + std::to_string(i_phase) + " is not allowed. Please specify the scaling manually, or do not specify it to use ones\n";
...

    {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong f.o. equations scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<fo_eqns.size(); i++) {
                MAVERICK_ASSERT(fo_eqns[i]>0,"Scaling for equation at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
            ...
            if (found) {
                found = checkIfStringMatchesAutomaticBounds(str);
                if (found) {
                    str = "Ocp scaling of type 'automatic-bounds' for first-order-equations for phase " + std::to_string(i_phase) + " is not allowed. Please specify the scaling manually, or do not specify it to use ones\n";
                    throw runtime_error(str);
                } else {
                    stringReplace(option_not_rec, "xxx", str);
                    throw runtime_error(option_not_rec);
    ...
    if (ocp_problem.numberOfPointConstraints(i_phase)>0) {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong point constraints scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<vec.size(); i++) {
                MAVERICK_ASSERT(vec[i]>0,"Scaling for point constraint at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
    ...
    if (ocp_problem.numberOfPathConstraints(i_phase)>0) {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong path constraints scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<vec.size(); i++) {
                MAVERICK_ASSERT(vec[i]>0,"Scaling for path constraint at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
    ...
    if (ocp_problem.numberOfIntConstraints(i_phase)>0) {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong integral constraints scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<vec.size(); i++) {
                MAVERICK_ASSERT(vec[i]>0,"Scaling for integral constraint at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
    ...
    if (ocp_problem.numberOfBoundaryConditions(i_phase)>0) {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong boundary conditions scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<vec.size(); i++) {
                MAVERICK_ASSERT(vec[i]>0,"Scaling for boundary condition at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
    ...
    if (ocp_problem.numberOfEventConstraints(i_phase)>0) {
        ...
        if (found) {
            ...
            if ( read != exp ) {
                throw runtime_error("Maverick ocp scaling: wrong event constraints scaling size at phase " + std::to_string(i_phase)
                                    + ". Found " + std::to_string(read) + " elements, expected " + std::to_string(exp) + "\n");
            }
            for (integer i=0; i<vec.size(); i++) {
                MAVERICK_ASSERT(vec[i]>0,"Scaling for event constraint at index " + std::to_string(i) + " at phase " + std::to_string(i_phase) + " cannot be non positive");
            }
    ...
    if (mesh_warning) {
        string str = "Ocp scaling for phase " + std::to_string(i_phase) + ": requested bounds-based scaling but no mesh has been set yet. Scaling for variables with mesh-dependent bounds may be inaccurate!\n";
...
vec_1d_real const & OcpScaling::getStatesControlScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _states_control.size(), "OcpScaling::getStatesControlScaling: phase index out of bounds.\n" );
    return _states_control[i_phase];
}
vec_1d_real const & OcpScaling::getAlgebraicStatesControlScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _algebraic_states_controls.size(), "OcpScaling::getAlgebraicStatesControlScaling: phase index out of bounds.\n" );
...
vec_1d_real const & OcpScaling::getPointConstraintsScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _point_constraints.size(), "OcpScaling::getPointConstraintsScaling: phase index out of bounds.\n" );
    return _point_constraints[i_phase];
}
vec_1d_real const & OcpScaling::getPathConstraintsScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _path_constraints.size(), "OcpScaling::getPathConstraintsScaling: phase index out of bounds.\n" );
    return _path_constraints[i_phase];
}
vec_1d_real const & OcpScaling::getIntConstraintsScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _int_constraints.size(), "OcpScaling::getIntConstraintsScaling: phase index out of bounds.\n" );
    return _int_constraints[i_phase];
}
vec_1d_real const & OcpScaling::getBoundaryConditionsScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _boundary_conditions.size(), "OcpScaling::getBoundaryConditionsScaling: phase index out of bounds.\n" );
    return _boundary_conditions[i_phase];
}
vec_1d_real const & OcpScaling::getEventConstraintsScaling(integer const i_phase) const {
    MAVERICK_ASSERT(i_phase < _event_constraints.size(), "OcpScaling::getEventConstraintsScaling: phase index out of bounds.\n" );
