...
namespace Maverick {
    ...
    MidpointOcpSolutionSinglePhase const & MidpointOcpSolution::operator()(integer const i_phase) const {
        MAVERICK_SKIPABLE_ASSERT(i_phase < (integer) _solutions.size(), "MidpointOcpSolutionSinglePhase::const operator(): number of mesh point " << i_phase << " greater than " << _solutions.size()-1 << ".")
        return _solutions[i_phase];
    }

    MidpointOcpSolutionSinglePhase & MidpointOcpSolution::operator()(integer const i_phase) {
        MAVERICK_SKIPABLE_ASSERT(i_phase < (integer) _solutions.size(), "MidpointOcpSolutionSinglePhase::operator(): number of mesh point " << i_phase << " greater than " << _solutions.size()-1 << ".")
        return _solutions[i_phase];
    }
    ...
    void MidpointOcpSolution::setSolutionAtPhase( integer const i_phase, MidpointOcpSolutionSinglePhase const & ocp_solution ) {
    ...
    void MidpointOcpSolution::evalAtMesh(integer const i_phase,
        ...
        evalAtMesh(i_phase,
                   ...
                   num_states_controls,  states_controls,       states_controls_upper_bounds_mult, states_controls_lower_bounds_mult,
                   num_alg_states_controls, algebraic_states_controls, algebraic_states_controls_upper_bounds_mult, algebraic_states_controls_lower_bounds_mult,
    ...
    void MidpointOcpSolution::evalAtMesh(integer const i_phase,
        ...
        MAVERICK_ASSERT( i_phase < getNumberOfPhases(), "MidpointOcpSolution::evalAtMesh: phase out of bounds. Requested phase " << i_phase << " when phase bounds are 0 - " << getNumberOfPhases()-1 << ".\n")
    ...
    void MidpointOcpSolution::eval(integer const i_phase,
        ...
        MAVERICK_ASSERT( i_phase < getNumberOfPhases(), "MidpointOcpSolution::eval: phase out of bounds. Requested phase " << i_phase << " when phase bounds are 0 - " << getNumberOfPhases()-1 << ".\n")
    ...
    void MidpointOcpSolution::writeOnePhaseMeshVarsToStream( integer const i_phase, ostream & out ) const {
        MAVERICK_ASSERT( i_phase < getNumberOfPhases(), "MidpointOcpSolution::writeMeshVarsToStream: phase out of bounds. Requested phase " << i_phase << " when phase bounds are 0 - " << getNumberOfPhases() -1<< ".\n")
