...
namespace Maverick {
    ...
    real MidpointMeshSinglePhase::getInitialZeta() const {
        MAVERICK_SKIPABLE_ASSERT( _number_of_mesh_points > 0, "MidpointMeshSinglePhase::getInitialZeta: mesh not initialized yet")
        return _zeta.front();
    }

    real MidpointMeshSinglePhase::getFinalZeta() const {
        MAVERICK_SKIPABLE_ASSERT( _number_of_mesh_points > 0, "MidpointMeshSinglePhase::getFinalZeta: mesh not initialized yet")
        return _zeta.back();
    }
    ...
    void MidpointMeshSinglePhase::setup( GC::GenericContainer const & mesh_data ) {
        ...
        if (!are_zeta_points_declared) { //if zeta points are NOT explicitely declared, check for segments
            ...
            if (are_segments_declared) {
                ...
                for (unsigned i_seg=0; i_seg<num_el; i_seg++) {
                    ...
                    // get length
                    real length;
                    MAVERICK_ASSERT(findRealFromGenericContainer(segment, "length", length), "length of segment not specified for segment at index " << i_seg << ".\n")
                    ...
                    MAVERICK_ASSERT( !(are_points_or_intervals_declared && is_grid_size_declared), "both number of intervals (or points) and grid size are specified for segment at index " << i_seg << ". Forbidden.\n")

                    MAVERICK_ASSERT( (are_points_or_intervals_declared || is_grid_size_declared), "neither number of intervals (or points) nor grid size are specified for segment at index " << i_seg << ". You must supply at least one.\n")

                    if (are_points_or_intervals_declared) {
                        if (are_num_points_declared && are_num_intervals_declared) {
                            MAVERICK_ASSERT(num_intervals == num_points-1, "both number of intervals and number of points points are specified for segment at index " << i_seg << ", but num_points != num_intervals+1. Forbidden.\n")
                        }
                        if (are_num_points_declared)
                            num_intervals = num_points-1;
                        MAVERICK_ASSERT( num_intervals > 0, "number of intervals must be greater than zero for segment at index " << i_seg << ".\n")
                        addSegment(Segment(length, num_intervals));
                    } else {
                        MAVERICK_ASSERT( grid_size > 0, "segment grid size must be greater than zero for segment at index " << i_seg << ".\n")
        ...
        MAVERICK_ASSERT( are_zeta_points_declared || are_segments_declared, "Mesh: either mesh points or segments must be supplied.\n")
        MAVERICK_ASSERT( isVectorIncreasingValues(_zeta.data(), (integer) _zeta.size()), "Mesh: non monotonically increasing indipendent coordinate.\n")
    ...
    real MidpointMeshSinglePhase::getZeta( integer const mesh_point_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_point_index < _number_of_mesh_points, "MidpointMeshSinglePhase::getZeta: number of mesh point " << mesh_point_index << " greater than " << _number_of_mesh_points-1 << ".")
        return _zeta[mesh_point_index];
    }

    real MidpointMeshSinglePhase::getZetaLeft( integer const mesh_interval_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_interval_index < _number_of_mesh_points-1, "MidpointMeshSinglePhase::getZetaLeft: number of mesh interval " << mesh_interval_index << " greater than " << _number_of_mesh_points-2 << ".")
        return _zeta[mesh_interval_index];
    }

    real MidpointMeshSinglePhase::getZetaRight( integer const mesh_interval_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_interval_index < _number_of_mesh_points-1, "MidpointMeshSinglePhase::getZetaRight: number of mesh interval " << mesh_interval_index << " greater than " << _number_of_mesh_points-2 << ".")
        return _zeta[mesh_interval_index+1];
    }

    real MidpointMeshSinglePhase::getZetaCenter( integer const mesh_interval_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_interval_index < _number_of_mesh_points-1, "MidpointMeshSinglePhase::getZetaCenter: number of mesh interval " << mesh_interval_index << " greater than " << _number_of_mesh_points-2 << ".")
    ...
    real MidpointMeshSinglePhase::getDz( integer const mesh_interval_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_interval_index < _number_of_mesh_points-1, "MidpointMeshSinglePhase::getDz: number of mesh interval " << mesh_interval_index << " greater than " << _number_of_mesh_points-2 << ".")
    ...
    real MidpointMeshSinglePhase::getDzDual( integer const mesh_point_index ) const {
        MAVERICK_SKIPABLE_ASSERT(mesh_point_index < _number_of_mesh_points, "MidpointMeshSinglePhase::getDzDual: number of mesh point " << mesh_point_index << " greater than " << _number_of_mesh_points << ".")
    ...
    MidpointMeshSinglePhase & MidpointMeshSinglePhase::operator << ( MidpointMeshSinglePhase const & mesh ) {
        ...
        MAVERICK_ASSERT( isVectorIncreasingValues( mesh_2_add.data(), (integer) mesh_2_add.size() ), "Mesh::operator << : non monotonically increasing zeta.\n")
    ...
    MidpointMeshSinglePhase::Segment::Segment( real const length, real const grid_size ) : _length(length), _num_intervals(0), _grid_size(grid_size), _segment_type(MidpointMeshSinglePhase::Segment::SegmentType::GridSize) {
    ...
    MidpointMeshSinglePhase::Segment::Segment( real const length, integer const num_intervals ) : _length(length), _num_intervals(num_intervals), _grid_size(0), _segment_type(MidpointMeshSinglePhase::Segment::SegmentType::NumPoints) {
        MAVERICK_ASSERT( _length > 0, "MidpointMeshSinglePhase::Segment: nonpositive segment length not allowed.\n")
        MAVERICK_ASSERT( _num_intervals > 0, "MidpointMeshSinglePhase::Segment: nonpositive segment number of intervals not allowed.\n")
    }
