...
void MidpointMesh::setup( GC::GenericContainer const & gc_mesh ) {
    ...
    if (gc_mesh.get_if_exists("log_factor", tmp)) {
        if ( tmp <= 0) {
            ...
            stringstream ss;
            ss << "MidpointMesh: non positive log factor is not allowed. Will use the defualt one: " << std::scientific << _new_points_log_factor << "\n";
    ...
    if (gc_mesh.get_if_exists("max_new_points", tmp_i)) {
        if (tmp_i < 1) {
            ...
            stringstream ss;
            ss << "MidpointMesh: non positive number of mesh points is not allowed. Will use the default one: " << _max_new_points << "\n";
            maverick.Log(InfoLevel::info_level_warning, ss.str() );
        } else {
...
std::unique_ptr<MeshSolutionRefiner> MidpointMesh::getMeshSolutionRefiner( MaverickOcp const & ocp_problem, OcpScaling const & ocp_scaling ) const {
...
MidpointMeshSinglePhase const & MidpointMesh::operator()(integer const i_phase) const {
    MAVERICK_ASSERT( i_phase < getNumberOfPhases(), "MidpointMesh::operator[]: phase out of bounds. Requested phase " << i_phase << " when phase bounds are 0 - " << getNumberOfPhases()-1 << ".\n")
