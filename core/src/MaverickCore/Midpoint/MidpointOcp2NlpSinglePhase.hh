...
namespace Maverick {
    ...
    protected:
        ...
        inline integer getNlpYPtrIndexForInterval( integer const mesh_interval ) const { return (_dim_y + _dim_ay) * mesh_interval; } // is not ok for last mesh point!!!
        ...
        inline integer getNlpTargetJacobianPtrIndexForInterval( integer const mesh_interval ) const  { return  (_target_j_xi_nnz - _lagrange_target_j_y_nnz) + (_lagrange_target_j_y_nnz + _lagrange_target_j_ay_nnz) * mesh_interval; }
        ...
        inline integer getNlpConstraintsPtrIndexForInterval( integer const mesh_interval ) const  { return _dim_q * mesh_interval; }

        inline integer getNlpConstraintsPtrIndexForLastPointConstr() const  { return getNlpConstraintsPtrIndexForInterval(_p_mesh->getNumberOfIntervals()); }

        inline integer getNlpConstraintsPtrIndexForIntConstr() const  { return getNlpConstraintsPtrIndexForLastPointConstr() + _dim_poc + _dim_bc; }

        inline integer getNlpConstraintsPtrIndexForBcs() const  { return _dim_q * _p_mesh->getNumberOfIntervals() + _dim_poc; }
        ...
        inline integer getNlpConstraintsJacobainMatrixYNnz() const { return (_fo_eqns_j_y_nnz + _path_constr_j_y_nnz)* 2 + _point_constr_j_y_nnz + _fo_eqns_j_ay_nnz + _path_constr_j_ay_nnz + _fo_eqns_j_p_nnz + _path_constr_j_p_nnz + _point_constr_j_p_nnz; }

        inline integer getNlpConstraintsJacobainMatrixFNnz() const { return _ocp_problem.boundaryConditionsJacXuInitNnz(_i_phase) + _ocp_problem.boundaryConditionsJacXuFinNnz(_i_phase) + _ocp_problem.boundaryConditionsJacPNnz(_i_phase); }

        inline integer getNlpConstraintsJacobainPtrIndexForInterval( integer const mesh_interval ) const { return getNlpConstraintsJacobainMatrixYNnz() * mesh_interval; }

        inline integer getNlpConstraintsJacobainPtrIndexForLastPointConstraints() const { return getNlpConstraintsJacobainPtrIndexForInterval(_p_mesh->getNumberOfIntervals()); }

        inline integer getNlpConstraintsJacobainPtrIndexForF() const { return getNlpConstraintsJacobainPtrIndexForLastPointConstraints() + _point_constr_j_y_nnz + _point_constr_j_p_nnz; }

        virtual integer getNlpConstraintsJacobainPtrIndexForIntConstr( integer const mesh_interval ) const { return getNlpConstraintsJacobainPtrIndexForF() +  getNlpConstraintsJacobainMatrixFNnz() + mesh_interval * (_int_constr_j_y_nnz + _int_constr_j_ay_nnz); }

        virtual integer getNlpConstraintsJacobainPtrIndexForIntConstrParams() const { return getNlpConstraintsJacobainPtrIndexForIntConstr(_p_mesh->getNumberOfIntervals()) + _int_constr_j_y_nnz; }
        ...
        inline integer getNlpHessianLeftColumnBlockNnz() const { return _hess_y_y_lower_mat_nnz + _hess_y_ay_mat_nnz + _hess_yleft_yright_mat_nnz + _hess_y_p_mat_nnz; }

        inline integer getNlpHessianCentreColumnBlockNnz() const { return _hess_ay_ay_lower_mat_nnz + _hess_y_ay_mat_nnz + _hess_ay_p_mat_nnz; }

        inline integer getNlpHessianRightColumnBlockNnz() const { return _hess_y_y_lower_mat_nnz + _hess_y_p_mat_nnz; }

        inline integer getNlpHessianFirstColumnBlockNnz() const { return _hess_first_y_y_lower_mat_nnz + _hess_y_ay_mat_nnz + _hess_yleft_yright_mat_nnz +  _hess_xi_xf_mat_nnz + _hess_first_y_p_mat_nnz; };

        inline integer getNlpHessianLastColumnBlockNnz() const { return _hess_last_y_y_lower_mat_nnz + _hess_last_y_p_mat_nnz; };

        inline integer getNlpHessianPtrIndexForLeftBlock( integer const mesh_interval ) const { return getNlpHessianFirstColumnBlockNnz() - getNlpHessianLeftColumnBlockNnz() + ( getNlpHessianLeftColumnBlockNnz() + getNlpHessianCentreColumnBlockNnz() ) * mesh_interval  ; }
        ...
        void calculateNlpQuantitiesBetweenMeshPoints(integer const first_mesh_point, integer const last_mesh_point,
                                                     ...
                                                     real   lagrange_target_jac_y_out[], real lagrange_target_j_r_out[], real lagrange_target_j_y_last_out[],
                                                     real   constraints_out[], real int_constraints_out[],
