...
void MidpointOcp2NlpSinglePhase::setupForNlpTargetGradient() {
    ...
    if (_is_gradient_dense) { //ipopt case
        ...
        for (integer i=0; i<(_p_mesh->getNumberOfIntervals()-1); i++) {
            sumAndWriteVectorTo(_p_lagrange_target_j_y_pattern, p_current_col_index, overall_offset, _lagrange_target_j_y_nnz);
        ...
        MAVERICK_DEBUG_ASSERT( overall_offset==getNlpSize(), std::string(__FILE__) + ": " + std::to_string(__LINE__) + ": wrong final overall offset value.\n")
    }
...
void MidpointOcp2NlpSinglePhase::calculateLagrangeTargetGradient(
    ...
    if (_is_gradient_dense) {
        ...
        for (integer i=0; i<lagrange_j_axu_nnz; i++) {
            *(p_gradient_left + _lagrange_target_j_y_nnz + lagrange_target_j_ay_pattern[i]) = tmp_target_j_axu[i] * _p_scale_factor_lagrange_target_j_ay[i] * d_zeta;
        }
        ...
        for (integer i=0; i<lagrange_j_p_nnz; i++) {
            *(p_gradient_p + lagrange_target_j_p_pattern[i]) += tmp_target_j_p[i] * _p_scale_factor_lagrange_target_j_p[i] * d_zeta;
        }
...
//        cout << endl;
        ...
        multiplyAndSumVectorTo(lagrange_j_y_left_mat.valuePtr(), p_gradient_left, _p_scale_factor_lagrange_target_j_y, _lagrange_target_j_y_nnz );
        multiplyVectorBy(tmp_target_j_axu, d_zeta, lagrange_j_axu_nnz);
        multiplyAndCopyVectorTo(tmp_target_j_axu, p_gradient_left + _lagrange_target_j_y_nnz, _p_scale_factor_lagrange_target_j_ay, lagrange_j_axu_nnz );
        multiplyAndCopyVectorTo(lagrange_j_y_right_mat.valuePtr(), p_gradient_right, _p_scale_factor_lagrange_target_j_y, _lagrange_target_j_y_nnz );
