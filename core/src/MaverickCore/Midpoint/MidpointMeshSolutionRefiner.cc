...
namespace Maverick {
    ...
    MidpointMeshSolutionRefiner::MidpointMeshSolutionRefiner(MaverickOcp const & ocp_problem, OcpScaling const & ocp_scaling) : MeshSolutionRefiner( ocp_problem, ocp_scaling) {};
    ...
    std::unique_ptr<Mesh> MidpointMeshSolutionRefiner::calculateMeshErrors(real const mesh_error_threshold,
        ...
        if ( solution_mesh.discretisationType() != DiscretisationType::midpoint )
            throw std::runtime_error("MidpointMeshSolutionRefiner::calculateMeshErrors: only a midpoint mesh can be anallysed");
        if ( sol.discretisationType() != DiscretisationType::midpoint )
            throw std::runtime_error("MidpointMeshSolutionRefiner::calculateMeshErrors: only a midpoint solution can be anallysed");
        ...
        for (integer i_phase = 0; i_phase < _ocp_problem.numberOfPhases(); i_phase++) {
            ...
            for (integer i_interval = 0; i_interval<mesh_errors.size(); i_interval++) {
                if (mesh_errors[i_interval] < 0) // in this case, an error occure, therefore we consider it to be greater than the mesh_error_threshold
    ...
    void MidpointMeshSolutionRefiner::calculateMeshErrorBetweenMeshPoints(integer const i_phase,
        ...
        for (integer i_interval = first_mesh_point; i_interval < last_mesh_point; i_interval++) {
            MAVERICK_DEBUG_ASSERT(i_interval<state_control[0].size()-1, "MidpointMeshSolutionRefiner::calculateMeshErrorBetweenMeshPoints: index of interval exceeds solution size\n")
            ...
            if (return_code != 0) {
                stringstream ss;
                ss << "Error in integrating mesh interval #" << i_interval << ". Will suppose integration error = error threshold * 1.01\n";
                MaverickSingleton::getInstance().Log(InfoLevel::info_level_warning, ss.str() );
                mesh_error[i_interval] = -1;
                continue;
            }
