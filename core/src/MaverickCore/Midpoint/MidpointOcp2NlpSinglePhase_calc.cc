...
integer MidpointOcp2NlpSinglePhase::calculateNlpQuantities(real const nlp_y[], integer const n_y, real const lambda[], real const lambda_0,
    ...
    MAVERICK_DEBUG_ASSERT( n_y == getNlpSize(), "MidpointOcp2NlpSinglePhase::calculateNLPQuantities: wrong length of NLP data. Passed " << n_y << ", actual " << getNlpSize());
    ...
    if (evaluate_target_j) {
        ...
        for (integer i_thread=0; i_thread<actual_num_threads; i_thread++) {
            ...
            writeRealToVector(t_lagrange_target_j_r_out[i_thread], 0, lagrange_jac_p_nnz); // write zero to the lagrange p gradient
    ...
    if (evaluate_target_j) {
        ...
        multiplyVectorBy(p_mayer_j_xup + mayer_j_xu_init_nnz + mayer_j_xu_fin_nnz, _p_scale_factor_mayer_j_p, mayer_j_p_nnz);
    }
    ...
    if (evaluate_constraints) {
        ...
        if ( _multiply_point_constr_by_dz )
            multiplyAndCopyVectorTo(_p_inv_scaling_point_constr_global, p_inv_scaling_point_constr, _p_mesh->getDzDual(_p_mesh->getNumberOfDiscretisationPoints()-1), _dim_poc);
        else
    ...
    if (evaluate_target_j) {
        ...
        {
            real * const p_target_j_y_last_out = target_jac_out + getNlpTargetJacobianPtrIndexForInterval(_p_mesh->getNumberOfIntervals());
    ...
    if (evaluate_constraints_j) {
...

        {
            ...
            real * int_constraints_jac_y_last_out = constraints_jac_out + getNlpConstraintsJacobainPtrIndexForIntConstr(_p_mesh->getNumberOfIntervals());
    ...
    if (evaluate_hessian) {
...
#endif
            hess_first_y_y_lower_mat = hess_y_y_lower_mat + bcs_hess_xu_init_xu_init_lower_mat + mayer_hess_xu_init_xu_init_lower_mat;
        ...
        }
        {
            ...
            real * p_hessia_written = hessian_out + getNlpHessianNnz() - _hess_p_p_lower_mat_nnz - getNlpHessianLastColumnBlockNnz();
#ifdef  MAVERICK_DEBUG
...
#endif
            hess_last_y_y_lower_mat = hess_y_y_lower_mat + bcs_hess_xu_fin_xu_fin_lower_mat + mayer_hess_xu_fin_xu_fin_lower_mat + last_point_constr_hess_y_y_lower_mat;
...
#endif
            hess_last_y_p_mat = hess_y_p_mat + bcs_hess_xu_fin_p_mat + mayer_hess_xu_fin_p_mat + last_point_constr_hess_y_p_mat;
...
#endif
            hess_p_p_final_lower_mat = hess_p_p_lower_mat + last_point_constr_hess_p_p_lower_mat + bcs_hess_p_p_lower_mat + mayer_hess_p_p_lower_mat;
...
void MidpointOcp2NlpSinglePhase::calculateNlpQuantitiesBetweenMeshPoints(integer const first_mesh_point, integer const last_mesh_point,
    ...
    try {
        ...
        for (integer current_mesh_interval = first_mesh_point; current_mesh_interval < last_mesh_point; current_mesh_interval++) {
            ...
            computeTpzCenter(p_left_state_control_scaled, p_right_state_control_scaled, _p_scaling_y, ocp_state_control, _dim_xu);
            ...
            computeTpzDerivative(p_left_state_control_scaled, p_left_state_control_scaled + _dim_ay + _dim_y, _p_scaling_y, ocp_state_control_derivative, 1/d_zeta, _dim_xu);
            ...
            if (hessian_out) {
                real * p_hessian_y_y_right_block = p_current_hessian + getNlpHessianLeftColumnBlockNnz() + getNlpHessianCentreColumnBlockNnz();
                real * p_hessian_y_p_right_block = p_hessian_y_y_right_block + _hess_y_y_lower_mat_nnz + _hess_y_ay_mat_nnz + _hess_yleft_yright_mat_nnz;
