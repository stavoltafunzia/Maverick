...
string MaverickOcp::algebraicStateName( integer const i_phase, integer i ) const {
    MAVERICK_ASSERT( i_phase >= 0 && i_phase < _num_p,
                         "in MaverickOcp::algebraicStateName( " << i_phase << " ) phase index out of range: [ 0.." << _num_p-1 << "]\n" ) ;
...
string MaverickOcp::algebraicControlName( integer const i_phase, integer i ) const {
    MAVERICK_ASSERT( i_phase >= 0 && i_phase < _num_p,
                         "in MaverickOcp::algebraicControlName( " << i_phase << " ) phase index out of range: [ 0.." << _num_p-1 << "]\n" ) ;
    MAVERICK_ASSERT( i >= 0 && i < _dim_au[i_phase],
                         "in MaverickOcp::algebraicControlName( " << i << " ) index out of range: [ 0.." << _dim_au[i_phase]-1 << "]\n" ) ;
...
string MaverickOcp::pathConstraintName( integer const i_phase, integer i ) const {
    MAVERICK_ASSERT( i_phase >= 0 && i_phase < _num_p,
                        "in MaverickOcp::pathConstraintName( " << i_phase << " ) phase index out of range: [ 0.." << _num_p-1 << "]\n" ) ;
...
string MaverickOcp::boundaryConditionName( integer const i_phase, integer i ) const {
    MAVERICK_ASSERT( i_phase >= 0 && i_phase < _num_p,
                         "in MaverickOcp::boundaryConditionName( " << i_phase << " ) phase index out of range: [ 0.." << _num_p-1 << "]\n" ) ;
    MAVERICK_ASSERT( i >= 0 && i < _dim_bc[i_phase],
                         "in MaverickOcp::boundaryConditionName( " << i << " ) index out of range: [ 0.." << _dim_bc[i_phase]-1 << "]\n" ) ;
...
string MaverickOcp::eventConstraintName( integer const i_phase, integer i ) const {
    MAVERICK_ASSERT( i_phase >= 0 && i_phase < _num_p,
                         "in MaverickOcp::eventConstraintName( " << i_phase << " ) phase index out of range: [ 0.." << _num_p-1 << "]\n" ) ;
...
void MaverickOcp::evalAtMesh(integer const i_phase,
    ...
    if ( (states_controls) || (states_controls_upper_bounds_mult) || (states_controls_lower_bounds_mult) ) {
        MAVERICK_ASSERT( num_states_controls == numberOfStates(i_phase) + numberOfControls(i_phase), "MaverickOcp::evalAtMesh: wrong number of states and controls.\n")
    }
    if ( (algebraic_states_controls) || (algebraic_states_controls_upper_bounds_mult) || (algebraic_states_controls_lower_bounds_mult) ) {
        MAVERICK_ASSERT( num_alg_states_controls == numberOfAlgebraicStates(i_phase) + numberOfAlgebraicControls(i_phase), "MaverickOcp::evalAtMesh: wrong number of algebraic states and controls.\n")
    }
    if ( fo_eqns_mult ) {
        MAVERICK_ASSERT( num_fo_eqns == numberOfFirstOrderEquations(i_phase), "MaverickOcp::evalAtMesh: wrong number of first order equation.\n")
    }
    if ( point_constr_mult ) {
        MAVERICK_ASSERT( num_point_constr == numberOfPointConstraints(i_phase), "MaverickOcp::evalAtMesh: wrong number of point constraints.\n")
    }
    if ( path_constr_mult ) {
        MAVERICK_ASSERT( num_path_constr == numberOfPathConstraints(i_phase), "MaverickOcp::evalAtMesh: wrong number of differential point constraints.\n")
    }
...
void MaverickOcp::eval(integer const i_phase,
    ...
    if ( boundary_conditions_mult ) {
        MAVERICK_ASSERT( num_boundary_conditions == numberOfBoundaryConditions(i_phase), "MaverickOcp::evalAtMesh: wrong number of boundary conditions.\n")
    }

    if ( int_constr_mult ) {
        MAVERICK_ASSERT( num_int_constr == numberOfIntConstraints(i_phase), "MaverickOcp::evalAtMesh: wrong number of integral constraints.\n")
    }
...
void MaverickOcp::writeSolutionToFile(OcpSolution const & solution, std::string const & filename) const {
    MAVERICK_ASSERT(solution.getNumberOfPhases() == numberOfPhases(), "MaverickOcp::writeSolutionToFile: solution provided has a wrong number of phases\n");
...
void MaverickOcp::translateSolutionHeader( std::stringstream const & input_header, integer const i_phase, std::ostream & output ) const {
...
void MaverickOcp::writeMeshHistoryToFile(SolverOutput const & solver_output, std::string const & filename) const {
