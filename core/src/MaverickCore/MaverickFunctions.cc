...
namespace Maverick {
    ...
    void printMaverickInfo(std::ostream & out) {
        ...
        string const developer = string("Developed by Nicola Dal Bianco - nic") + string("ola.d") + string("albian") + string("co@g") + string("mai") + string("l.com");
        char const border = '*';

        out << endl;
        out << std::left << std::setfill(border) << std::setw(width) << "" << endl;
        out << border; centerStringInStream(out, "" , width-2, ' '); out << border << endl;
        out << border; centerStringInStream(out, header, width-2, ' '); out << border << endl;
        // out << border; centerStringInStream(out, version, width-2, ' '); out << border << endl;
        out << border; centerStringInStream(out, "" , width-2, ' '); out << border << endl;
        out << border; centerStringInStream(out, "" , width-2, ' '); out << border << endl;
        out << border; centerStringInStream(out, developer, width-2, ' '); out << border << endl;
        out << border; centerStringInStream(out, "" , width-2, ' '); out << border << endl;
        out << std::left << std::setfill(border) << std::setw(width) << "" << endl;
        out << endl;
    }
    ...
    std::unique_ptr<OcpSolution> convertGuessTable2OcpSolution( vector<real_table> const & tables, MaverickOcp const & ocp_problem, std::vector<std::vector<std::string>> & found_variables ) {

        found_variables = {};
        MidpointOcpSolution * solution = new MidpointOcpSolution();
        for (integer i=0; i<tables.size(); i++) {
            std::vector<string> tmp_found;
            std::unique_ptr<OcpSolutionSinglePhase> c_sol = convertGuessTable2OcpSolutionSinglePhase(tables[i], ocp_problem, i, tmp_found);
            solution->operator<<( *((MidpointOcpSolutionSinglePhase*) c_sol.get() ));
            found_variables.push_back(tmp_found);
        }
        return std::unique_ptr<OcpSolution>(solution);
    }
    ...
    std::unique_ptr<OcpSolutionSinglePhase> convertGuessTable2OcpSolutionSinglePhase( real_table const & table, MaverickOcp const & ocp_problem, integer const i_phase, std::vector<std::string> & found_variables ) {
        ...
        zeta_center.push_back( *(zeta_center.end()-1) );

        vec_1d_real zeros_1d = vec_1d_real(zeta.size(), 0);
        vec_2d_real point_constr = vec_2d_real( ocp_problem.numberOfPointConstraints(i_phase),  zeros_1d  );
        vec_2d_real path_constr = vec_2d_real( ocp_problem.numberOfPathConstraints(i_phase),  zeros_1d  );
        vec_2d_real int_constr = vec_2d_real( ocp_problem.numberOfIntConstraints(i_phase),  zeros_1d  );
        vec_2d_real fo_eqns = vec_2d_real( ocp_problem.numberOfStates(i_phase),  zeros_1d  );
        vec_1d_real boundary_conditions = vec_1d_real( ocp_problem.numberOfBoundaryConditions(i_phase), 0);
        vec_2d_real post_processing = vec_2d_real( ocp_problem.numberOfPostProcessing(i_phase),  zeros_1d  );
        vec_2d_real differential_post_processing = vec_2d_real( ocp_problem.numberOfDifferentialPostProcessing(i_phase),  zeros_1d  );
        vec_2d_real integral_post_processing = vec_2d_real( ocp_problem.numberOfIntegralPostProcessing(i_phase),  zeros_1d  );

        vec_2d_real states_controls = vec_2d_real( ocp_problem.numberOfStates(i_phase)+ocp_problem.numberOfControls(i_phase),  zeros_1d  );
        vec_2d_real algebraic_states_controls = vec_2d_real( ocp_problem.numberOfAlgebraicStates(i_phase)+ocp_problem.numberOfAlgebraicControls(i_phase),  zeros_1d  );
        ...
        for (i_s = 0; i_s < ocp_problem.numberOfAlgebraicStates(i_phase); i_s++) {
            try {
                ...
                vec_1d_real const & vec = table.at(name);
                algebraic_states_controls[ i_s ] = vec;
                found_variables.push_back(name);
            } catch (...) {}
        }
        for (integer i_c = 0; i_c < ocp_problem.numberOfAlgebraicControls(i_phase); i_c++) {
            try {
                ...
                vec_1d_real const & vec = table.at(name);
                algebraic_states_controls[ i_c + i_s ] = vec;
                found_variables.push_back(name);
            } catch (...) {}
        }
    ...
    //TODO: find multiplier in guess
        ...
        sol->setSolution(0, //target
                        zeta, //zeta_center,
                        zeros_1d, zeros_1d, //cumulative and integrand target
