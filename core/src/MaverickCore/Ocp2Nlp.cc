...
integer Ocp2Nlp::getNlpTargetGradientPattern( integer col_index[], integer const col_offset, integer const length ) const {
        MAVERICK_DEBUG_ASSERT( length == getNlpTargetGradientNnz(), "Ocp2Nlp::getNlpTargetGradientPattern: wrong number of nnz.\n")
        MAVERICK_DEBUG_ASSERT( _p_nlp_target_j_cols != nullptr, "Ocp2Nlp::getNlpTargetGradientPattern: called when _p_nlp_target_j_cols is nullptr.\n")

        sumAndWriteVectorTo(_p_nlp_target_j_cols, col_index, col_offset, length);
        return 0;
}

integer Ocp2Nlp::getNlpConstraintsJacobianPattern( integer row_index[], integer col_index[], integer const row_offset, integer const col_offset, integer const length ) const {
...
integer Ocp2Nlp::getNlpHessianPattern( integer row_index[], integer col_index[], integer const row_offset, integer const col_offset, integer const length ) const {
#ifdef MAVERICK_DEBUG
...
void Ocp2Nlp::evalNlpConstraints(Nlp & nlp) const {
    ...
    MAVERICK_ASSERT(nlp.getNlpConstraintsSize() == getNlpConstraintsSize(), "Ocp2Nlp::evalNlpConstraints: wrong nlp constraints size")
