...
namespace MaverickUtils {
    ...
    real ComponentBase::findValueParamWithPrefixAndName(string const &prefix, string const &name, GenericContainer const &gc) const {
		...
		if (foundRoot && foundGrouped) { // if the parameter is found twice, accept the values only if they are equal, otherwise thorw exception
		...
		throw runtime_error(" can't find parameter, or wrong parameter type, for name '" + name +"', group '" + prefix +"' in setup.\n");
	}

	int_type ComponentBase::findIntParamWithPrefixAndName(string const &prefix, string const &name, GenericContainer const &gc) const {
		bool foundRoot = false;
		int_type rootValue;
		try { //check if the parameter is found in the root container
			if (prefix.compare("")!=0)
	...
	real ComponentBase::findValueParamWithPrefixAndName(string const &prefix, string const &name1, string const &name2, GenericContainer const &gc) const {
		real value;
		exception _exc;
		DoubledValue *_dv=0;
		...
		if (foundFirst && foundSecond) // if the parameter is found with different names thorw exception
			throw runtime_error(_model + ": parameter of group '" + prefix + "' defined with both names: '" + name1 +"', and '" + name2 +"' in setup.\n");

		if (foundFirst || foundSecond) { // if the parameter is found only once, throw an exception if an exception was detected, otherwise return the value
	...
	real ComponentBase::findValueParamWithNameAndInverseNameAndDefValue(string const &name1, string const &name2, real const defValue, GenericContainer const &gc) const {
		real value = defValue;
		...
		if (foundFirst && foundSecond) {// if the parameter is found with different names thorw exception
			string errorString = _model + ": parameter of name '" + name1 + "' defined with both with its name and its inverse name: '" + name2 + "' in the setup. Only one definition is allowed.\n";
    ...
    string ComponentBase::findStringParamWithPrefixAndName(std::string const &prefix, std::string const & name, GC::GenericContainer const & gc) const {
        bool foundRoot = false;
        string rootValue;
        try { //check if the parameter is found in the root container
            if (prefix.compare("")!=0)
    ...
    vec_real_type ComponentBase::findVectorParamWithPrefixAndName(string const &prefix, string const &name, GenericContainer const &gc) const {
		bool foundRoot = false;
		...
		// if the parameter is not found throw exception
		throw runtime_error("Can't find vector parameter, or wrong parameter type, for name '" + name + "', group '" + prefix + "' in setup.\n");
	}
